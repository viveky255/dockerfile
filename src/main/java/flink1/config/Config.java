...
public enum Config implements PeConfig {
  ...
  private final static String SERVICE_ID = "pe/flink1.processor.flink";

  Config() {
    config = SpConfig.getSpConfig(SERVICE_ID);
    config.register(ConfigKeys.HOST, "flink", "Data processor host");
    config.register(ConfigKeys.PORT, 8090, "Data processor port");
    config.register(ConfigKeys.SERVICE_NAME, "flink", "Data processor service name");
